...
internal sealed class CorDebugWrapperService(
    ILoggingService log,
    LogStore logStore) : ICorDebugWrapper
{
        ...
        string coreclrPath, ulong baseAddress)
    {
        ...
        try
        {
            ...
            model.Process = OpenVirtualProcessImpl(
                coreclrPath, baseAddress, dataTarget, _log, _logStore,
                out var clrVersion);
    ...
    public void RefreshModules(CorDebugWrapperModel model)
    {
        ...
        try
        {
            ...
            catch (Exception ex) { _log.LogInfo(_logStore, $"  ProcessStateChanged failed: {ex.Message}"); }
