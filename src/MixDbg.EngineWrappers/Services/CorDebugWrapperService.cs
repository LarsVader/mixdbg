...
internal sealed class CorDebugWrapperService : ICorDebugWrapper
{
    ...
    //
    // Historical note: an ICorDebug ILFrame-based fast path was attempted first
    // (InitializeManagedLocals + FindILFrame). It failed at runtime because the
    // piggybacked V4 process throws E_NOTIMPL on chain/frame enumeration. See
    // docs/failed-approaches.md. Locals are now produced exclusively by the SOS
    // path on the caller side; this wrapper just stores the pre-formatted result.

    public int StoreSimpleLocals(CorDebugWrapperModel model, VariableInfo[] locals)
        => locals.Length == 0 ? 0 : model.ManagedVariables.Allocate(locals);

    public VariableInfo[] GetManagedVariables(CorDebugWrapperModel model, int variablesReference)
        => model.ManagedVariables.Get(variablesReference) ?? [];
